...
type BasketItem = { gtin: string; qty: number };
type Candidate = { gtin: string; score: number; reason: string[] };

export async function recommend(items: BasketItem[], customerId: string | null, top = 5) {
  ...
  const byLift: Candidate[] = coocs.map(c => ({
    gtin: c.gtin_b,
    score: c.lift,
  ...
  if (customerId) {
    const rows = await prisma.$queryRawUnsafe<{ gtin: string; freq: number }[]>(`
  ...
  for (const [gtin, c] of map) {
    ...
    if (boost) map.set(gtin, { ...c, score: c.score + boost, reason: [...c.reason, `rule:+${boost}`] });
  }
